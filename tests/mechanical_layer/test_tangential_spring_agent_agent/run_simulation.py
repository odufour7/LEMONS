"""Test script for running the mechanical layer engine in a rolling without sliding agent agent scenario."""
...
Ndt = 120  # How many dt will be performed in total
...
copyfile("../initial_agent_dynamics_files/AgentDynamics_test_tangential_spring_agent_agent.xml", "dynamic/AgentDynamics.xml")
...
for t in range(Ndt):
    ...
    for agent in agentsTree.findall("Agent"):
        ...
        # look for ID attribute in agent tag
        agent_id = agent.get("Id")
        # is it is equal to 4, set the force and moment to specific values
        if agent_id == "2":
            dyn_elem.set("Fp", "120.0,40")
            dyn_elem.set("Mp", "0.0")
        if agent_id == "1" or agent_id == "0":
            dyn_elem.set("Fp", "0.0,50.0")
            dyn_elem.set("Mp", "0.0")
